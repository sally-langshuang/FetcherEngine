...
func getGraphLink(t TreeNode, elemWidth int) (graph Graph) {
	...
	if reflect.ValueOf(t.GetLeft()).IsNil() && reflect.ValueOf(t.GetRight()).IsNil() {
		return tg
	}
	...
	tg.offSet = (lo + ro) / 2
	rootLine := layElem(tg.offSet, gc.Width(), elemWidth, t.GetVal())
...
func MaxVal(t TreeNode) (res int) {
	res = t.GetVal()
	if !reflect.ValueOf(t.GetLeft()).IsNil() {
		res = max(res, MaxVal(t.GetLeft().(TreeNode)))
	}
	if !reflect.ValueOf(t.GetRight()).IsNil() {
		res = max(res, MaxVal(t.GetRight().(TreeNode)))
	}
	return res
}
...
func InitTree(t TreeNode, s string) TreeNode {
	numArr := exactInt(s)
	if len(numArr.arr) == 0 {
		return nil
	}
	root := t.Constructor(*numArr.arr[0]).(TreeNode)
	i := 1
	for parentLevel := []TreeNode{root}; len(parentLevel) > 0; {
		...
		for _, n := range parentLevel {
			if i < len(numArr.arr) {
				if numArr.arr[i] != nil {
					n.SetLeft(t.Constructor(*numArr.arr[i]).(TreeNode))
